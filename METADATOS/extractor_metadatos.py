...
def _obtener_metadatos_resolucion(logical_screen_descriptor: bytearray) -> dict:
...
def leer_paleta_global(file, packed: bytearray) -> dict:
...
def leer_bloque_imagen(file) -> dict:
    ...
    while True:
        tamanio_subbloque = struct.unpack("B", file.read(1))[0]
        if tamanio_subbloque == 0:
            break
        file.read(tamanio_subbloque)  # Leer pero no almacenar los datos
        total_datos_comprimidos += (
            tamanio_subbloque  # Contar el tamaño de cada subbloque
        )

    # Guardamos solo el tamaño total de los datos comprimidos
    imagen_data["tamaño_total_datos_comprimidos"] = total_datos_comprimidos
    return imagen_data


def leer_control_grafico(file) -> dict:
...
def leer_comentario(file) -> dict:
    comentarios = []
    while True:
        tamanio_subbloque = struct.unpack("B", file.read(1))[0]
        if tamanio_subbloque == 0:
            break  # Fin del bloque de comentario
        comentarios.append(file.read(tamanio_subbloque))
...
def leer_gif(gif_path) -> dict:
    ...
    with open(gif_path, "rb") as file:
        ...
        metadatos["pantalla"] = _obtener_metadatos_resolucion(logical_screen_descriptor)
        ...
        while True:
            ...
            elif bloque_inicial == SEPARADORES_BLOQUES["bloque_inicial"]:  # Extensiones
                extension_label = file.read(1)
                if extension_label == SEPARADORES_BLOQUES["control_grafico"]:
                    cantidad_frames += 1
                    bloques.append(leer_control_grafico(f))
                elif extension_label == SEPARADORES_BLOQUES["comentario"]:
...
#     pprint(gif_metadatos, stream=f)
#     print("Archivo 'res.txt' guardado con éxito.")
